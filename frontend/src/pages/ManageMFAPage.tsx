...
import { disableTwoFactor, enableTwoFactor, getTwoFactorStatus, resetRecoveryCodes } from '../lib/authAPI';
          ...
          <div className="card shadow-sm mb-4">
            <div className="card-body p-4">
              ...
              {isAuthenticated && twoFactorStatus ? (
                <>
                  <div className="mb-3">
                    <span className={`badge rounded-pill ${twoFactorStatus.isTwoFactorEnabled ? 'text-bg-success' : 'text-bg-warning'}`}>
                      {twoFactorStatus.isTwoFactorEnabled ? 'MFA enabled' : 'MFA not enabled'}
                    </span>
                  </div>
                    ...
                    <div className="col-md-7">
                      {!twoFactorStatus.isTwoFactorEnabled ? (
                        <form onSubmit={handleEnable}>
                          <div className="mb-3">
                            <label className="form-label" htmlFor="authenticatorCode">Authenticator code</label>
                            <input
                              id="authenticatorCode" type="text" className="form-control"
                              inputMode="numeric" autoComplete="one-time-code"
                              value={authenticatorCode} onChange={(e) => setAuthenticatorCode(e.target.value)} required
                            />
                          </div>
                        ...
                        <div>
                          <p className="mb-3">MFA is active. Use authenticator codes at login or fall back to a recovery code.</p>
                          <div className="d-flex gap-2 flex-wrap">
                            <button type="button" className="btn btn-outline-secondary" onClick={handleResetRecoveryCodes} disabled={isSubmitting}>
                              Reset recovery codes
                            </button>
                            <button type="button" className="btn btn-outline-danger" onClick={handleDisable} disabled={isSubmitting}>
                              Disable MFA
                            </button>
                          </div>
                        </div>
                  ...
                  {recoveryCodes.length > 0 ? (
                    ...
                    <p className="mt-4 mb-0 text-muted">Recovery codes left: {twoFactorStatus.recoveryCodesLeft}</p>
                  )}
                </>
              ) : null}
            </div>
